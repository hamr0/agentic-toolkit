...
`packages/<kit>` at liteagents v3.12.0 (content diff, zero tracked
...
- **`/branch-review`'s and `/refactor`'s ledger `change` count no longer
  ...
  *first* physical line, when the real trailing tag (on the bullet's *last*
  ...
  that the tail stays unwrapped on a bullet's last line. `docs/branch-review-README.md`
  and a stale "`/debrief` ... before commit" line in the same file are
  updated to match.
